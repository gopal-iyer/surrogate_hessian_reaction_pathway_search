...
# for a factor of N, the DMC run will be executed for Nx longer
# CAUTION ! Set to 1 unless you're absolutely sure
dmc_enhancement_factor = 64
...
for iter_i in range(n_iter):
    ...
    for int_str in range(n_intermediate):
        ...
        #GRI the following 2 lines are from Juha; use in case of "W, sigma not found" error
        for i in range(len(subspace_hessian.Lambda)):
    ...
    for int_str in range(n_intermediate):
        true_idx = int_str + 1
        write_str = str(updated_structures[true_idx].params) + "\n"
        f_log.write(write_str)
    f_log.write("************************************\n")
