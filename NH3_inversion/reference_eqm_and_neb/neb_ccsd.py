...
for index in range(3):
    calc = PySCF()
    j = -4 + index
    # borrowing initial guess from DFT NEB pathway, fmax=0.02
    image_fname = f'NEBf0.01.traj@{j}'
    image = io.read(image_fname, parallel=False)
...
neb = NEB(images)
opt = BFGS(neb, trajectory='NEB_ccsd_f0.01.traj', logfile='NEB_ccsd_f0.01.log')
opt.run(fmax=0.01)
